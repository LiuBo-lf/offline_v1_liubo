...
SELECT
    tm_id,
    tm_name,
    NVL((repurchase_users * 100.0) / total_users, 0) AS repurchase_rate -- 使用浮点数确保精确计算
FROM (
         SELECT
             tm_id,
             tm_name,
             COUNT(user_id) AS total_users, -- 每个用户仅出现一次，无需DISTINCT
             SUM(CASE WHEN purchase_count >= 2 THEN 1 ELSE 0 END) AS repurchase_users -- 统计复购用户数
         FROM (
                  SELECT
                      tm_id,
                      tm_name,
                      user_id,
                      COUNT(*) AS purchase_count -- 计算每个用户的购买次数
                  FROM dwd_trade_order_detail
...
drop table if exists gmall.adr_number_of_times_purchase;
create table if not exists adr_number_of_times_purchase(
                                                           purchase_times_range string,
                                                           user_count integer
)row format delimited fields terminated by "\t"
    stored as orc
    location "/warehouse/gmall/ads/adr_number_of_times_purchase"
    tblproperties ("orc.compress" = "snappy");

-- 统计用户购买次数的分布（如购买1次、2次的用户数）
INSERT OVERWRITE TABLE adr_number_of_times_purchase
SELECT
    purchase_times_range,
...
FROM (
         SELECT
             ...
             CASE
                 ...
                 ELSE '5次以上'
                 END AS purchase_times_range
         FROM (
                  SELECT
                      user_id,
                      COUNT(DISTINCT order_id) AS purchase_count  -- 用户总购买次数
                  FROM dwd_trade_order_detail
                  GROUP BY user_id
              ) t1
     ) t2
GROUP BY purchase_times_range;
select * from  adr_number_of_times_purchase;
--优惠活动与优惠券分摊金额占比
select * from ads_percentage_total_sales;
drop table if exists gmall.ads_percentage_total_sales;
create table if not exists ads_percentage_total_sales(
                                                         tm_id string,
                                                         tm_name string,
                                                         total_activity_amount string,
                                                         total_coupon_amount string,
                                                         activity_ratio  string,
                                                         coupon_ratio string
)row format delimited fields terminated by "\t"
    stored as orc
    location "/warehouse/gmall/ads/ads.percentage_total_sales"
    tblproperties ("orc.compress" = "snappy");
-- 统计优惠活动和优惠券分摊金额占总销售额的比例
INSERT OVERWRITE TABLE ads_percentage_total_sales
SELECT
    tm_id,
    tm_name,
    SUM(split_activity_amount) AS total_activity_amount,  -- 总活动优惠金额
    SUM(split_coupon_amount) AS total_coupon_amount,      -- 总优惠券金额
    ROUND(
            SUM(split_activity_amount) * 100.0 / SUM(order_price * sku_num),
            2
    ) AS activity_ratio,  -- 活动优惠占比
    ROUND(
            SUM(split_coupon_amount) * 100.0 / SUM(order_price * sku_num),
            2
    ) AS coupon_ratio     -- 优惠券占比
FROM dwd_trade_order_detail
GROUP BY tm_id, tm_name;
select * from ads_percentage_total_sales;
